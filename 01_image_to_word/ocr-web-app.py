...
def word_segmentation(image_path, output_image_path):
    ...
    for file in os.listdir(cropped_dir):
     file_path = os.path.join(cropped_dir, file)
     try:
    ...
    # Sonucu gösterme
    st.image(image, caption='Seçilen Görüntü', use_column_width=True)
    st.success('Görsel işleme tamamlandı.')

def main():
    st.title("Ocr App")
    st.markdown('### Görüntüden Metne Dönüşüm ')

    # Dosya seçme butonu
    uploaded_file = st.file_uploader("OCR Gerçekleştirilecek  Görseli Seçiniz!", type=["jpg", "png", "jpeg"])
    ...
    if uploaded_file is not None:
        ...
        # Tahminlerin olduğu dosyayı oku ve ekrana yaz
        st.markdown('### Elde Edilen Metin')
        try:
            with open(output_text_path, "r") as f:
                predictions_content = f.read()
                st.text(predictions_content)
