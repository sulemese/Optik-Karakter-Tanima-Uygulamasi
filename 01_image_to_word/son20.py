...
def line_segmentation(image_path, output_path):
    ...
    # Dikey histogramı çıkar
    vertical_proj = np.sum(eroded, axis=0)
    
    # Dikey histogram grafiğini çiz
    plt.figure(figsize=(10, 5))
    plt.plot(vertical_proj, range(eroded.shape[1]))
    plt.gca().invert_yaxis()  # Grafiği ters çevir
    plt.title('Dikey Histogram')
    plt.xlabel('Piksel Sayısı')
    plt.ylabel('Sütunlar')
    plt.grid()
    plt.savefig(output_path + "_vertical_histogram.png")
    plt.show()
    ...
    # Sonucu kaydetme
    cv2.imwrite(output_path + "_segmented_image.jpg", image)
...
# Örnek kullanım
input_image_path = "C:/a/24.jpg"
output_image_path = "C:/a/23_segmented"
